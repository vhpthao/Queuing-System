...
import {State as NoticeState, TypeDataThongBao, fetchDataFromFirebase} from '../store/noticeSlice'
...
function Header() {
  ...
  const breadcrumbNames = [
    ...
    { path: 'device', name: 'Thiết bị > Danh sách thiết bị', class: 'breadcrumb-item-gray breadcrumb-item-orange' },
    { path: 'addDevice', name: '> Thêm thiết bị' },
    { path: 'detailDevice', name: '> Chi tiết thiết bị' },
    { path: 'updateDevice', name: '> Cập nhật thiết bị' },

    { path: 'service', name: 'Dịch vụ > Danh sách dịch vụ' },
    { path: 'addService', name: '> Thêm dịch vụ' },
    { path: 'updateService', name: '> Cập nhật dịch vụ' },
    { path: 'detailService', name: '> Chi tiết dịch vụ' },

    { path: 'giveNumber', name: 'Cấp số > Danh sách cấp số' },
    { path: 'detailGiveNumber', name: '> Chi tiết cấp số' },
    { path: 'giveNewNumber', name: '> Cấp số mới' },

    { path: 'report', name: 'Báo cáo > Danh sách báo báo' },

    { path: 'manaRole', name: 'Cài đặt hệ thống > Quản lý vai trò' },
    { path: 'updateRole', name: '> Cập nhật vai trò' },
    { path: 'addRole', name: '> Thêm vai trò' },

    { path: 'manaAccount', name: 'Cài đặt hệ thống > Quản lý tài khoản' },
    { path: 'updateAccount', name: '> Cập nhật tài khoản' },
    { path: 'addAccount', name: '> Thêm tài khoản' },

    { path: 'userLog', name: 'Cài đặt hệ thống > Nhật ký người dùng' },
    { path: 'information', name: 'Thông tin cá nhân' },

    
  ];

  const renderBreadcrumb = () => {
    const isUpdatingPage = path.includes('detailDevice') 
                        || path.includes('detailService')
                        || path.includes('detailGiveNumber')
                        || path.includes('updateDevice')
                        || path.includes('updateService')
                        || path.includes('updateRole')
                        || path.includes('updateAccount'); 
...
const thongBao = useSelector((state: RootState) => state.notice.thongBaoList);
  ...
  return (
    <div style={{display:'flex'}}>
      ...
      <Popover placement="bottomRight" title={text} content={content} trigger="click"  overlayClassName="custom-popover" style={{ borderRadius: '8px' }}>
        <Button className='chuong' style={{borderRadius:'300px'}}>
       <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none" >
        ...
        <div style={{display:'flex', marginRight:'100px', marginLeft:'10px'}}>
          ...
          <div style={{ marginTop: '5px' }}>
            ...
            <p className='line2'><Link to={'/information'} className='link'>{hoTen}</Link></p>
          </div>
