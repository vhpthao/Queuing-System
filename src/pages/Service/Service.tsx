...
import { fetchDataFromFirebaseDV, State as ServiceState } from '../../store/serviceSlice'
...
const columns: ColumnsType<DataTypeDV> = [
  ...
  },
  {
    ...
    render: (_, record) => (
      <Space size="middle">
        <Link to={`/service/detailService/${record.key}`} style={{ textDecoration: 'underline' }}>Chi tiết</Link>
      </Space>
    ),
  },
  {
    ...
    render: (_, record) => (
      <Space size="middle">
        <Link to={`/service/updateService/${record.key}`} style={{ textDecoration: 'underline' }}>Cập nhật</Link>
      </Space>
    ),
  },
];
...
function Service() {
  ...
  const filteredData = filterData(service.serviceList);
