...
import { fetchDataFromFirebaseCS, selectCS } from '../../store/giveNumberSlice';
...
function DetailGiveNumber() {
  ...
  const selectedGiveNumber = useSelector((state: any) => state.giveNumber.selectedGiveNumber);

  useEffect(() => {
    if (selectedGiveNumber) {
      setTenKH(selectedGiveNumber.tenKH || 'Huỳnh Lam Minh');
      setTenDV(selectedGiveNumber.tenDV || '');
      setSdt(selectedGiveNumber.sdt || '0183718334');
      setNguonCap(selectedGiveNumber.nguonCap || '');
      setEmail(selectedGiveNumber.email || 'minh@gmail.com');
      setTrangThai(selectedGiveNumber.trangThai || '');
      setTgCap(selectedGiveNumber.tgCap || '');
      setNgayCap(selectedGiveNumber.ngayCap || '');
      setTgSD(selectedGiveNumber.tgSD || '');
      setNgaySD(selectedGiveNumber.ngaySD || '');
      setStt(selectedGiveNumber.stt || '');

      console.log(selectedGiveNumber.tenDV)
        ...
        <div style={{ display: 'flex' }}>
          <section className='scTTB'>
            ...
            <div style={{ display: 'flex' }}>
              ...
              <p>{getStatusIcon(trangThai)} {trangThai}</p>
            </div>
            <div style={{ display: 'flex' }}>
          ...
          <Button className='btnQuayLaiCTCS' style={{marginLeft:'30px'}}>
            <Link to={''}>
