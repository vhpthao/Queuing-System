...
const columns: ColumnsType<TableDataItemGiveNumber> = [
  ...
  },
  {
    ...
    render: (_, record) => (
      <Space size="middle">
        <Link to={`/giveNumber/detailGiveNumber/${record.key}`} style={{ textDecoration: 'underline' }}>
          Chi tiết
        </Link>
      </Space>
    ),
...
function GiveNumber() {
  ...
  const handleCapSoMoi = () => {
    navigate('/giveNumber/giveNewNumber');
  }
  ...
  value={ngaySD ? (ngaySD) : null}/>
          ...
          <div style={{ display: 'flex' }}>
            ...
            <Button className='btnTTB' onClick={handleCapSoMoi} style={{marginLeft:'60px'}}>
