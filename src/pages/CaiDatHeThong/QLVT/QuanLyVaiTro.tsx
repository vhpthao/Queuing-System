...
import { fetchDataFromFirebase, TableDataItemVaiTro, State as VaiTroState } from '../../../store/vaiTroSlice'
...
const columns: ColumnsType<TableDataItemVaiTro> = [
  {
    title: 'Tên vai trò',
    dataIndex: 'tenVT',
    key: 'tenVT',
  },
  {
    ...
    dataIndex: 'soND',
    key: 'soND',
    render: (_, record: TableDataItemVaiTro) => record.soND, // Hiển thị số người dùng
  },
  {
    title: 'Mô tả',
    dataIndex: 'moTa',
...

  {
    ...
    render: (_, record) => (
      <Space size="middle">
        <Link to={`/qlvt/capnhatvt/${record.key}`} style={{ textDecoration: 'underline' }}>Cập nhật</Link>
      </Space>
    ),
  },
];
...
function QuanLyVaiTro() {
  ...
  const vaiTro = useSelector((state: RootState) => state.vaiTro);
  ...
  const handleThemVaiTroQLVT = () => {
    navigate('/qlvt/themvt');
  }

  // lọc dữ liệu
  const [filterKeyword, setFilterKeyword] = useState<string>('');

  const filterData = (dataSource: TableDataItemVaiTro[]): TableDataItemVaiTro[] => {
  ...
  const filteredData = filterData(vaiTro.vaiTroList);
  ...
  const getCurrentPageData = (data: TableDataItemVaiTro[]): TableDataItemVaiTro[] => {
  ...
  `}
        ...
        <div>
          ...
          <div style={{ display: 'flex' }}>
            ...
            <Button className='btnTTB' onClick={handleThemVaiTroQLVT}>
