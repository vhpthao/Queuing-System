...
import { fetchDataFromFirebase, State as ThietBiState } from '../../store/thietBiSlice'
import { Dispatch } from 'redux';
import Header from '../../layout/Header'

interface RootState {
  thietBi: ThietBiState;
}
...
const columns: ColumnsType<DataType> = [
  ...
  },
  {
    ...
    render: (_, record) => (
      <Space size="middle">
        <Link to={`/thietbi/cttb/${record.key}`} style={{ textDecoration: 'underline' }}>Chi tiết</Link>
      </Space>
    ),
  },
  {
    ...
    render: (_, record) => (
      <Space size="middle">
        <Link to={`/thietbi/capnhattb/${record.key}`} style={{ textDecoration: 'underline' }}>Cập nhật</Link>
      </Space>
    ),
  },
];
...
function ThietBi() {
  ...
  const thietBi = useSelector((state: RootState) => state.thietBi);
    ...
    return dataSource.filter((TableDataItem) => {
      ...
      if (filterKeyword) {
        ...
        return (
          ...
          TableDataItem.dvSD.toLowerCase().includes(lowerCaseKeyword)
        );
      }
  ...
  const filteredData = filterData(thietBi.thietBiList);
  ...
  useEffect(() => {
    // Lưu giá trị mặc định vào localStorage khi thay đổi
