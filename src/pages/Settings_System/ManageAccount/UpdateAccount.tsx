...
import { fetchNguoiDungDataFromFirebase, selectND } from '../../../store/userSlice';
...
function UpdateAccount() {
  ...
  .then((docSnapshot) => {
    if (docSnapshot.exists()) {
      ...
      dispatch(selectND({ ...selectedUser, ...updatedData }) as any);
          setHoTen('');
          setTenDN('');
          setEmail('');
          setMatKhau('');
  ...
  useEffect(() => {
    if (selectedUser) {
      ...
      setTenDN(selectedUser.tenDN || '');
      setHoTen(selectedUser.hoTen || '');
      setSdt(selectedUser.sdt || '')
      setMatKhau(selectedUser.matKhau || '');
      setTrangThaiHD(selectedUser.trangThaiHD || '');
      setVaiTro(selectedUser.vaiTro || '');
    }
  }, [selectedUser]);
