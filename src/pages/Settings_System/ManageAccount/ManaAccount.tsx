...
import { fetchNguoiDungDataFromFirebase, TableDataItemUser, NguoiDungState as UserState } from '../../../store/userSlice'
...
const columns: ColumnsType<TableDataItemUser> = [
  {
    title: 'Tên đăng nhập',
    dataIndex: 'tenDN',
    key: 'tenDN',
  ...
  },
  {
    ...
    render: (_, record) => (
      <Space size="middle">
        <Link to={`/manaAccount/updateAccount/${record.key}`} style={{ textDecoration: 'underline' }}>Cập nhật</Link>
      </Space>
    ),
  },
];
...
function ManaAccount() {
  ...
  const handleThemTK = () => {
    navigate('/manaAccount/addAccount');
  }
  ...
  const filterData = (dataSource: TableDataItemUser[]): TableDataItemUser[] => {
  ...
  //lọc tài khoản
  const filteredData = filterData(user.userList);
  ...
  const getCurrentPageData = (data: TableDataItemUser[]): TableDataItemUser[] => {
  ...
  return (
    <div style={{ display: 'flex' }}>
      <Navbar />
      <div className='right' style={{width: '1231px', height:'740px'}}>
        <style>
          {`
    .ant-table-thead th {
          ...
          <div style={{ display: 'flex' }}>
            ...
            <div className='kcVaiTro'>
              ...
              <Select
                ...
                options={loaiVaiTro}
                size='large'
                defaultValue={defaultFilterVaiTro}
                onChange={(value) => {
                  setFilterVaiTro(value === 'Tất cả' ? null : value);
