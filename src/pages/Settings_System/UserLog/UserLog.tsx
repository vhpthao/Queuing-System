...
import { State as UserLogState, TableDataItemUserLog, fetchDataFromFirebase } from '../../../store/userLogSlice'
...
interface RootState {
  userLog: UserLogState;
}

const columns: ColumnsType<TableDataItemUserLog> = [
  {
    title: 'Tên đăng nhập',
    dataIndex: 'tenDN',
    key: 'tenDN',
  },
  {
    ...
    dataIndex: 'tgTD',
    key: 'tgTD',
    render: (text: string, record: TableDataItemUserLog) => {
...
function UserLog() {
  ...
  const userLog = useSelector((state: RootState) => state.userLog);
  ...
  const filterData = (dataSource: TableDataItemUserLog[]): TableDataItemUserLog[] => {
  ...
  const filteredData = filterData(userLog.userLogList);
  ...
  const getCurrentPageData = (data: TableDataItemUserLog[]): TableDataItemUserLog[] => {
